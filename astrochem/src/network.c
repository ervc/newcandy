...
unsigned int get_species_mass_and_charge( char* species, double* mass, int* charge )
{
  ...
  *charge = 0;
  ...
  while( *specie_pt != '\0' )
      ...
      // 1->9 element multiplier
      else if( *specie_pt >= 49 && *specie_pt <= 57 )
        {
          ...
          if( element_idx == 0 || mass_multiplier != 1 )
            {
              fprintf (stderr, "astrochem: error: this specie: %s is not correctly written 1\n", species );
              return 0;
            }
          ...
          switch( *specie_pt )
            ...
            case(45):
              *charge = -1;
              break;
            default:
              fprintf (stderr, "astrochem: error: this specie: %s is not correctly written 2\n", species );
              return 0;
              break;
            }
          break;
	...
	{
	  fprintf (stderr, "astrochem: error: this specie: %s is not correctly written 3\n", species );
	  return 0;
	}
    }

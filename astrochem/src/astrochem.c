...
#endif
  fprintf (stdout, "Copyright (c) 2006-2019 Sebastien Maret\n");
  fprintf (stdout, "\n");
  fprintf (stdout,
            ...
            int verbose)
{
  ...
  /* Compute the minimum density to set the absolute tolerance of the
     solver */
  min_nh = cell->nh[0]; // note cell = cell_table_t, so cell->nh[0] is nh in zeroeth cell
  if (mode == DYNAMIC)
    {
      int i;
      ...
      for (i = 0; i < ts->n_time_steps; i++)
        {
          ...
          if (i!=0 && mode == DYNAMIC)
            { //if Dynamic then reset cell_unik to whatever cell we happen to be looking at
                      ...
                      if( specie_in_products )
                        {
                          ...
                          //   || network->reactions[k].reaction_type == 20
                          //   || network->reactions[k].reaction_type == 21
                          //   || network->reactions[k].reaction_type == 25
                          //   || network->reactions[k].reaction_type == 13
                          //   || network->reactions[k].reaction_type == 16
                          //   || network->reactions[k].reaction_type == 17
                          //   || network->reactions[k].reaction_type == 18
                          //   || network->reactions[k].reaction_type == 90
                          //   || network->reactions[k].reaction_type == 91
                          //   || network->reactions[k].reaction_type == 92)
                          //   {
                          //     formation_route.rate = astrochem_mem.params.reac_rates[k];
                          //     // formation_route.rate *=
                          //      // NV_Ith_S (astrochem_mem.y, network->reactions[k].reactants[0]);
